...
def plot_final_comparison():
    ...
    mode1_file = res_dir / "1b_1m_HeadOrtho" / "results.json"
    mode3_file = res_dir / "1b_1m_Mode3" / "results.json"
    base_file = res_dir / "1b_1m_Baseline" / "results.json"
    
    data = {}
    for name, fpath in [("Mode1", mode1_file), ("Mode3", mode3_file), ("Baseline", base_file)]:
        if fpath.exists():
    ...
    colors = {'Mode1': '#a855f7', 'Mode3': '#facc15', 'Baseline': '#22d3ee'} # Purp, Yellow, Cyan
    styles = {'Mode1': 'o', 'Mode3': '^', 'Baseline': 's'}
    ...
    ax.set_title('Loss Comparison (10M Tokens)')
    ax.legend(facecolor='#21262d', edgecolor='#30363d', labelcolor='#c9d1d9')

    # PR
    ax = axes[1]
    for name, d in data.items():
        # Trim PR to match loss len if needed, or just plot all
    ...
    out_path = res_dir / "mode1_v_mode3_v_baseline.png"
