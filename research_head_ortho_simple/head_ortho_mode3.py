...
def setup_head_ortho_optimizer(model: nn.Module, config: LLMConfig, use_head_ortho: bool = True):
    ...
    if head_ortho_params:
        head_ortho_group = {
            ...
            "nesterov": True,
            "ns_steps": 5,
            "ortho_mode": 3,  # mode-3: Orthogonalize HEADS against each other
            "head_info": {
...
def run_experiment(use_head_ortho: bool) -> dict:
    ...
    """
    tag = "Mode3" if use_head_ortho else "Baseline"
    run_name = f"1b_1m_{tag}"
    print(f"\n{'='*70}")
    print(f"  EXPERIMENT: {run_name} (Mode-3 Head Ortho)")
    print(f"{'='*70}")
...
if __name__ == "__main__":
    ...
    # Run: Mode 3 Only (Combine later)
    print("\n\n🔬 PHASE 1: Training WITH HeadOrtho Mode 3...")
    res_mode3 = run_experiment(use_head_ortho=True)

    # Note: We already have Baseline and Mode1 results in other folders.
    # We will write a specific plotter to combine all 3.
